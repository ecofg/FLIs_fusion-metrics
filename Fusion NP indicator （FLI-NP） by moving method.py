...
# coding=UTF-8
'''
The fusion NP indicator based on G1,G2 LULC
...
outpathNP=r""#Output result path file
outpathPD=r""
rs=1#resolution (km)
...
def nuMclass(g1n,g2n):
    ...
    return list
def numPatch(arr):
    arraster=arcpy.NumPyArrayToRaster(arr)
    fc = r"arrasterTopolygon"
    arcpy.RasterToPolygon_conversion(arraster,fc, "NO_SIMPLIFY","VALUE")
    num_patch = []
    rows = arcpy.da.SearchCursor(fc, ["gridcode"])
    for row in rows:
        num_patch.append(rows[0])
    arcpy.Delete_management(fc)
    return Counter(num_patch)

def moveWindow(dfclp):
    ...
    arrcomp = np.array(dfcomp)
    del dfcomp, dfwindow
    return arrcomp

def piWindow(arr):
    arrcomp = arr.flatten().astype(float)
...
if __name__ == '__main__':
    ...
    for l in range(lie - 2 * r):
        ...
        for h in range(hang - 2 * r):
            ...
            arrg1=moveWindow(dfg1clp)
            arrg2=moveWindow(dfg2clp)
            pig1=piWindow(arrg1)
            pig2=piWindow(arrg2)
            nppatchg1=numPatch(arrg1)
            sumg1=0
            nums=0
            for g1,num in zip(g1class,numberlist):
                ...
                if pig1[g1]==0:
                    ...
                    sumg1+=nppatchg1.get(int(g1))
                else:
                    for g2 in g2class[nums-num:nums]:
                        if pig2[g2]!=0:
                            sumg2+=pig2[g2]*math.log(pig2[g2])
                    sumg1+=nppatchg1.get(int(g1))*(1-pig1[g1]*sumg2/math.log(num))
    ...
    fusionRaster.save(outpathNP)
    cnum = dfno[-np.isnan(dfno)].tolist
    A = math.sqrt(rs) * cnum
    FLI_PRD=fusionRaster/A
    FLI_PRD.save(outpathPD)
    print ("FLI_NP & FLI_PD are calculated successfully")
    del gc.garbage[:]
    gc.collect()
